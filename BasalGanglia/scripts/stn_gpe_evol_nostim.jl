using Distributions, Random, Statistics, FileIO, JLD2, Distributed, DifferentialEquations, Plots, BlackBoxOptim, DSP
...
# start optimization
opt = bbsetup(stn_gpe_loss; Method=method, Parameters=p, SearchRange=(collect(zip(p_lower,p_upper))), NumDimensions=length(p), MaxSteps=250, workers=workers(), TargetFitness=0.0, PopulationSize=10000, CallbackFunction=cb, CallbackInterval=1.0)
